...
export function checkpointTool(options: Readonly<{
  ...
  return tool({
    description: "Compare independent advisor proposals and batch-record checked findings at a normal verification checkpoint. " +
    ...
    async execute(args, context) {
      ...
      if (options.config.abort_on_blocker && args.phase === "before_action" && paused.length > 0) {
